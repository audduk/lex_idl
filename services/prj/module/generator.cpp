...
QString genHFuncArgs(int id, makeNameFunc func) {
  QString res;
...
QString genCppFuncArgs(int id, makeCppFunc func) {
  QString res;
  for (uint j=id+1; j<syn_idents.size() && syn_idents[id].scope<syn_idents[j].scope; ++j) {
    //для всех непосредственных потомков элемента (аргументы команды)
    const CIdentInfo& loc = syn_idents[j];
...
void genService(int id, int strId) {
  ...
  const CIdentInfo& ident = syn_idents[id];
  if (cpp.size() > 0)
    resultCpp += makeScope(ident.name(), "namespace", cpp, 0);
  if (h.size() > 0)
    resultH += makeScope(ident.name(), "namespace", h, ident.scope);
}

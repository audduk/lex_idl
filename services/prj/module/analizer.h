...
enum IdentType {
  ...
  itStruct, itEnum,                   // составные типы
  itFloat, itInt, itString, itChar,   // простые типы
  itFunction,                         // функции
  itConst, itService, itArg           // прочие элементы
};

inline bool isSimpleType(IdentType it) { return it==itFloat || it==itInt || it==itString || it==itChar; }
inline bool isComplexType(IdentType it) { return it==itStruct || it==itEnum; }
...
struct CIdentInfo {
  ...
  int scope;      //!< уровень идентификатора
...
//! Добавление новой константы типа itString
int appendString(const char*);
